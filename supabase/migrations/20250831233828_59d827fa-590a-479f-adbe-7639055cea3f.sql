-- Fix project deletion by removing ALL conflicting triggers and consolidating equipment update logic

-- Drop ALL existing triggers on loans table to start fresh
...
DROP TRIGGER IF EXISTS sync_equipment_loan_status_trigger ON public.loans;
DROP TRIGGER IF EXISTS update_equipment_simplified_status_trigger ON public.loans;

-- Now create only the essential triggers with consolidated logic
-- 1. Equipment loan status sync (handles equipment reference fields only)
