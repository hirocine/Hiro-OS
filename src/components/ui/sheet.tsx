...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-[64] bg-background/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
    ref={ref}
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      {/* Handle bar iOS para bottom sheet */}
      {side === "bottom" && (
        <div className="mx-auto mt-2 mb-3 h-1.5 w-12 shrink-0 rounded-full bg-muted-foreground/40" />
      )}
      {children}
      {/* Só mostra X se NÃO for bottom */}
      {side !== "bottom" && (
