...
function CaseCard({ item, idx }: { item: CaseItem; idx: number }) {
  const [thumbUrl, setThumbUrl] = useState<string | null>(null)

  useEffect(() => {
    if (!item.vimeoId) return

    const hashSegment = item.vimeoHash ? `/${item.vimeoHash}` : ''
    const oembedUrl = `https://vimeo.com/api/oembed.json?url=https://vimeo.com/${item.vimeoId}${hashSegment}`
    ...
    fetch(oembedUrl)
      ...
      .catch(() => {
        setThumbUrl(`https://vumbnail.com/${item.vimeoId}.jpg`)
      })
  }, [item.vimeoId, item.vimeoHash])
  ...
  return (
    ...
    >
      {/* Thumbnail always visible as poster */}
      {thumbUrl && (
        <img
          src={thumbUrl}
          alt={item.titulo || ''}
      ...
      {/* Video iframe on top - hidden in print */}
      {item.vimeoId && (
        <iframe
  ...
  return (
    <section className='pt-6 pb-10 md:pt-8 md:pb-14 proposal-content-px'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        {cases.map((item, idx) => (
          <CaseCard key={item.id || idx} item={item} idx={idx} />
        ))}
      </div>
    </section>
  )
