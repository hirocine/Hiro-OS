...
function PdfHero({ proposal }: { proposal: Proposal }) {
  ...
  return (
    <div style={{ padding: '80px 60px 50px', display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: PAGE_MIN_HEIGHT }}>
...
function PdfClients() {
  ...
  return (
    <div style={{ padding: '50px 60px 40px' }}>
      ...
      <h2 style={{ fontSize: 32, fontWeight: 700, marginBottom: 30, fontFamily: '"Helvetica Now Display", "Helvetica Neue", Helvetica, Arial, sans-serif' }}>
        Nossos Clientes
      </h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 20, alignItems: 'center', justifyItems: 'center' }}>
        {logos.map((logo, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 50 }}>
            <img src={logo} alt='' style={{ maxHeight: 40, maxWidth: 100, objectFit: 'contain', opacity: 0.7 }} />
          </div>
        ))}
      </div>
    </div>
